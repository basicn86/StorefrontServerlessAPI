...
```bash
StorefrontSAM$ sam local invoke NetCodeWebAPIServerless --event events/event.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` command to run the API locally on port 3000.

```bash
StorefrontSAM$ sam local start-api
StorefrontSAM$ curl http://localhost:3000/
```
...
```bash
StorefrontSAM$ sam logs -n NetCodeWebAPIServerless --stack-name StorefrontSAM --tail
```
...
```bash
StorefrontSAM$ dotnet test test/ServerlessAPI.Tests/ServerlessAPI.Tests.csproj 
```
...
```bash
sam delete --stack-name StorefrontSAM
```
